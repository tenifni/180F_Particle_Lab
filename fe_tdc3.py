...
cut = 100         # Don't fit times below this value
channel = 2       # TDC 4,5,6, or 7
cl = 0            # Number of neutron counter coincidences we require
...
# fit
expo = lambda x, a, tau, offset: a*np.exp(-x/tau)+offset
p0 = [np.max(y), 500, 0]
...
plt.axhline(pfit[2],label='tail',color='m')
plt.axvspan(spike[0],spike[1], alpha=0.2, color='red')
plt.text(5e2,30,'tau = {0:.1f} +- {1:.1f}'.format(4*pfit[1],4*perr[1]))
plt.ylim(-5, 50)
plt.xlim(0,4000)
